...
写文章只需要关注`docs/.vuepress/public/`和有编号的文件夹，如`01.新生入学/`，其余内容请自行查看相关项目文档或者询问AI，但是修改需谨慎。
...
├─ .github/
│  └─ workflows/
│     └─ static.yml                 # GitHub Actions：自动构建并发布到 GitHub Pages
├─ docs/                            # 站点正文，最重要的目录
│  ├─ .vuepress/                    # VuePress 站点配置
│  │  ├─ config.js                  # 全站核心配置：导航、侧边栏、插件、SEO、主题
│  │  ├─ enhanceApp.js              # VuePress 运行时增强
│  │  ├─ config/
│  │  │  ├─ baiduCode.js            # 百度统计等配置
│  │  │  └─ htmlModules.js          # 可复用 HTML 模块配置
│  │  ├─ components/
│  │  │  └─ DownloadPageImage.vue   # 自定义 Vue 组件
│  │  ├─ plugins/
│  │  │  └─ love-me/                # 自定义插件
│  │  ├─ public/                    # 静态资源，构建时原样拷贝
│  │  │  ├─ css/                    # 额外样式资源
│  │  │  ├─ img/                    # 图片资源
│  │  │  ├─ markmap/                # 思维导图导出的 html / md 文件
│  │  │  ├─ CNAME                   # 自定义域名配置
│  │  │  └─ robots.txt              # 搜索引擎抓取配置
│  │  └─ styles/
│  │     ├─ index.styl              # 页面样式覆盖
│  │     └─ palette.styl            # 主题色等样式变量
│  ├─ index.md                      # 首页内容
│  ├─ @pages/                       # 分类页、标签页、归档页
│  │  ├─ archivesPage.md
│  │  ├─ categoriesPage.md
...
│  ├─ 03.学习资料/                  # 校区学习资料汇总
│  ├─ 04.群汇总/                    # 同好群、老乡群
│  ├─ 05.校园生活/                  # 账号、网站、电话、教室、自习、美食、医保等
│  ├─ 06.就业/                      # 求职、内推、就业信息
│  ├─ 07.计算机知识专题/            # Python、Git、Vim、C++ 环境等
│  └─ 10.贡献/                      # 给贡献者看的协作说明
├─ utils/                           # 辅助脚本，不是站点正文
│  ├─ check.js                      # Windows 下运行脚本前做提示检查
│  ├─ config.yml                    # 批量修改 frontmatter 的配置文件
│  ├─ editFrontmatter.js            # 批量增删改 Markdown frontmatter
│  └─ modules/
│     ├─ fn.js                      # 工具函数
│     └─ readFileList.js            # 递归读取 Markdown 文件
├─ package.json                     # 项目依赖与 npm 脚本入口
├─ package-lock.json                # 依赖锁定文件
├─ README.MD                        # 项目说明
├─ LICENSE                          # 开源协议
├─ .gitignore                       # Git 忽略规则
└─ node_modules/                    # npm 安装的依赖，自动生成，一般不手改
```
...
同时，此wiki也有较为完全的[Git使用文章](/pages/GitStudy/)
